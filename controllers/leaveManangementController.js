...
exports.getLeave = (req, res, next) => {
    connection.query(`select * from leaves where leave_id = ${req.params.id}`, (error, results) => {
        if (error) res.json({status: "error", result: error});
        res.json({status: "ok", result: results});
    });
};
...
exports.updateLeave = (req, res, next) => {
    ...
    const queryString = "update leaves " +
...
exports.deleteLeave = (req, res, next) => {
    connection.query(`delete from leaves where id = ${req.params.id}`, (error, results) => {
        if (error) res.json({status: "error", result: error});
        res.json({status: "ok", result: results});
    });
}