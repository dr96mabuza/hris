...
exports.createDocument = async (req, res, next) => {
  ...
  const query =
    ...
    `values( '${getDocumentName}', ${converted.toString()},  ${req.body.employeeId}, '${req.body.documentName}')`;
    ...
    (error, results) => {
      ...
      if (results.length > 0) {
        for (key in results[0]) {
          if (key === "document") {
            results[0][key] = new Uint8Array(
              new ArrayBuffer(
                results[0][key].split(",").map((item) => Number(item)).length,
              ),
            );
          }
        }
